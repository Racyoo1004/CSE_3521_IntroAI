...
def calc_jacobian(X,p):
    ...
    der_a = arrayX ** b
    der_b = a * der_a * np.log(arrayX)
    der_c = arrayX
    der_d = np.ones((N, 1))

    der_a = der_a.reshape(N, 1)
    der_b = der_b.reshape(N, 1)
    der_c = der_c.reshape(N, 1)
    der_d = der_d.reshape(N, 1)
...
def nonlinear_regression_gn(X, Y, initialP):
    ...
    for iteration in range(GAUSS_NEWTON_ITERATIONS): #PLEASE do not change this line
        ...
        jTyDiff = np.matmul(J.transpose(), y_diff)
